...
def query(
    ...
    try:
        if pr is not None and not repo:
        ...
        if decisions:
            ...
            rows = get_pr_decisions(connection, repo=repo, pr_number=pr)
            console.print(render_decisions(rows, repo=repo, pr_number=pr))

        if health:
            if pr is None or repo is None:
                raise typer.BadParameter("--repo and --pr are required with --health")
            summary = get_pr_health(connection, repo=repo, pr_number=pr)
