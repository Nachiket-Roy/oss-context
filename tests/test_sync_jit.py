...
import pytest
from oss_context.models import IssueData, PullRequestData, RepoRef
from oss_context.sync import ensure_issue_synced, ensure_pr_synced
...
async def test_ensure_pr_synced_stale(tmp_path):
    ...
    with (
        ...
        mock_conn.execute.return_value.fetchone.return_value = {"updated_at": "2020-01-01T00:00:00Z"}
...
async def test_ensure_pr_synced_up_to_date(tmp_path):
    ...
    with (
        ...
        mock_conn.execute.return_value.fetchone.return_value = {"updated_at": "2022-01-01T00:00:00Z"}
