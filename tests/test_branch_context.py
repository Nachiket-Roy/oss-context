"""Tests for branch-aware context resolution.
    ...
    def runner(args: list[str], cwd: Path | None, allow_failure: bool):
        ...
        if args[:4] == ["gh", "pr", "view", "--json"]:
            if gh_output is None:
                raise AssertionError("gh fallback should not be used in this test")
            return gh_output
...
def _seed_branch_fixture(connection) -> None:
    ...
    connection.execute(
        ...
        VALUES(3, 'thread-3', 1, NULL, NULL, 'active', NULL, NULL, ?, ?)
        """,
        ((now - timedelta(hours=6)).isoformat(), now.isoformat()),
    )
    connection.execute(
        """
        INSERT INTO review_comments(
            ...
            3, 3, 5003, 'dana', 'General feedback.',
            ?, ?, 0, 0, 0, 'QUESTION', 0.55
        )
        """,
    ...
    connection.execute(
        ...
        INSERT INTO llm_cache(
            ...
            'QUESTION', 'General feedback', 0.55, ?
        )
        """,
        (now.isoformat(),),
    )
...
def test_open_pr_is_preferred_over_closed_branch_reuse(tmp_path):
    connection = DatabaseManager(tmp_path / "oss_context.db").initialize()
    try:
        _seed_branch_fixture(connection)
        now = datetime.now(UTC)
        connection.execute(
            ...
            VALUES(
                2, 102, 1, 7, 'Old auth', 'closed', 'alice',
                ?, ?, 'body', 'main', 'feature/auth', NULL
            )
            """,
...
def test_gh_fallback_resolves_pr_url(tmp_path):
    connection = DatabaseManager(tmp_path / "oss_context.db").initialize()
    try:
        runner = _fake_runner_factory(
            tmp_path,
            ...
            "https://github.com/acme/widgets.git",
            gh_output=(
                '{"number":42,"title":"Auth","state":"OPEN",'
                '"url":"https://github.com/acme/widgets/pull/42",'
                '"headRefName":"feature/auth","baseRefName":"main"}'
            ),
        )
        resolved = resolve_branch_pr(connection, runner=runner)
